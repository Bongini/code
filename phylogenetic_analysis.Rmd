...
Had ambigueties from 1073-1724. So I only kept the beginning of the sequence to avoid the ambigueties.
...
All the KF66 and KF88 sequences originated from the same study done in India.

Omega-3 fatty acid production from enzyme saccharified hemp hydrolysate using a novel marine thraustochytrid strain by Gupta,A., Abraham,R.E., Barrow,C.J. and Puri,M.
...
All originate from the same paper, done by the same authors of the KF66 sequences.
...
Pollen baiting facilitates the isolation of marine thraustochytrids with potential in omega-3 and biodiesel by Gupta,A., Abraham,R.E., Barrow,C.J. and Puri,M.production
...
All originating from the same study in India and in the trial 2 tree it made a strange clade together with long branches. BLAST results are around 90% identity.

Decided to remove them all.

![](images/clipboard-2862485564.png)
...
Article: Investigating Microbial Eukaryotic Diversity from a Global Census: Insights from a Comparison of Pyrotag and Full-Length Sequences of 18S rRNA Genes

### AB695493_1_1790_U_Labyrinthulaceae_ANT10_3_sp

Outlier in tree. BLAST results showed low per. identity. Results were all other unclultured eukaryotes or not Labyrinthulomycetes.

Article: Eukaryotic phylotypes in aquatic moss pillars inhabiting a freshwater lake in East Antarctica, based on 18S rRNA gene analysis
...
Article: Microbial communities of the disappearing periglacial environment on top of Mount Kilimanjaro

### AY180038_1_1725_U_Thraustochytriaceae_X_sp

Outlier in tree. BLAST results showed low per. identity (under 80%). From a very old study (2003). Uncultured eukaryote

Article: Novel eukaryotic lineages inferred from small-subunit rRNA analyses of oxygen-depleted marine environments

### GM664571_1_1753_U_Aurantiochytrium_limacinum

Outlier in tree. BLAST results did not show query and the results showed low per. identitiy. No article, just a patent from India.
...
Extreme outlier in tree. Poor BLAST results. From a Chinese paper.
...
All outliers together in a very defined clade. Poor BLAST results, all come from the same university in Thailand in 2006.... Directly submitted and no paper. Some appear to have a intron in the beginning.
...
Outlier in tree, poor BLAST results. Indian study
...
From the paper Strains of heterotrophic microalgae rich in PUFA isolated from Mangalavanam mangrove habitat, Cochin, Kerala.
...
## Importing metabarcodes

```{r}

asvs <- read_excel("raw_data/metabarcode/asv_updated.xlsx")

#for my alignment I have used all seqs classified as Labyrithulomycetes in the class column, but for my tree I am only using the seqs with class_boot >= 80
asvs_laby <- asvs %>%
  filter(class == "Labyrinthulomycetes") %>%
  filter(class_boot >= 80)
  

asvs_laby <- asvs_laby %>%
  mutate(asv_name = paste(asv_code, species, sep = "_"))

asvs_laby$asv_name <- asvs_laby$asv_name %>%
  str_remove_all("[.]")
```

## Exporting metabarcodes

```{r}
asv_out <- Biostrings::DNAStringSet(asvs_laby$sequence)

names(asv_out) <- asvs_laby$asv_name

Biostrings::writeXStringSet(asv_out, "output/metabarcode/asvs_laby.dfasta", compress = FALSE, width=20000)
```
